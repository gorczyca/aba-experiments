...
def plot_cactus_plot_reversed(solved_dfs):
    ...
    for solver, solver_df in solved_dfs.items():
        ...
        # just a scatter plot
        #ax = group_df.plot(kind='scatter', y='int_index', x=new_column_name, s=20.0, color=ct.SCATTER_COLORS_DICT[solver], marker=ct.MARKERS_DICT[solver], ax=ax, label=solver)
